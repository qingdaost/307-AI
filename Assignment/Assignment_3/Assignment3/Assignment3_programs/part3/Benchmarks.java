...
///// Setup
    ...
    public void setup(EvolutionState state, Parameter base)
        ...
        String problem = null;  // = state.parameters.getString(base.push(P_PROBLEM_TYPE), null);
        int benchmark = -1;
                ...
                try
                    ...
                    while(scan.hasNextDouble())
                        ...
                        for(int i = 0; i < numInputs; i++)
                            ...
                            else state.output.fatal("Non-normal number of in data points in training file ", base.push(P_TRAINING_FILE), null);
                            }
                        ...
                        else state.output.fatal("Non-normal number of out data points in training file ", base.push(P_TRAINING_FILE), null);
                        input.add(in);
        ...
        if (problem == null)  // it's being loaded from file
            {
            boolean found = false;
            System.out.println(trainingInputs[0].length);
...
///// Evaluation.  evaluate(...) uses training cases, and describe(...) uses testing cases
    ...
    public void evaluate(EvolutionState state, Individual ind, int subpopulation, int threadnum)
        ...
        if (!ind.evaluated)  // don't bother reevaluating
            ...
            for (int y=0;y<trainingInputs.length;y++)
                ...
                if ((input.x < 0 && trainingOutputs[y] == -1) || (input.x > 0 && trainingOutputs[y] == 1))
                	hits++;
    ...
    public void describe(EvolutionState state, Individual ind, int subpopulation, int threadnum, int log)
        ...
        for (int y=0;y<testingInputs.length;y++)
            ...
            //final double HIT_LEVEL = 0.01;
            //if (error <= HIT_LEVEL) hits++; 
            if ((input.x < 0 && testingOutputs[y] == -1) || (input.x > 0 && testingOutputs[y] == 1))
            	hits++;
