...
def training_process(filename):
    ...
    R2 = RL2[-1] - RL2[0]
    R3 = RL3[-1] - RL3[0]
    return (training_list, R0, R1, R2, R3)
...
def error_rate(training_list, test_list, R0, R1, R2, R3):
    """Calculate the error rate of test data"""
    error = 0
    for test in test_list:
        min_dist = distance_measure(test[0], training_list[0][0], R0, R1, R2, R3)
        Iris_class = training_list[0][1]
        for training in training_list:
            dist = distance_measure(test[0], training[0], R0, R1, R2, R3)
            if min_dist > dist:
                min_dist = dist
                Iris_class = training[1]
        if Iris_class != test[1]:
            error += 1
    return error / len(test_list)
...
def main():
    """ Gets the job done """
    (training_list, R0, R1, R2, R3) = training_process("iris-training.txt")
    test_list = test_process("iris-test.txt")
    #error_rate2 = error_rate(training_list, test_list, R0, R1, R2, R3)
    #print ("k = 1, accuracy rate is " + str(1 - error_rate2))
    
    printresult(training_list, R0, R1, R2, R3, test_list, 1)
    
    printresult(training_list, R0, R1, R2, R3, test_list, 3)
    #simpleresult(training_list, R0, R1, R2, R3, test_list, 1)
    #simpleresult(training_list, R0, R1, R2, R3, test_list, 3)
    #simpleresult(training_list, R0, R1, R2, R3, test_list, 5)
    #simpleresult(training_list, R0, R1, R2, R3, test_list, 7)
    #simpleresult(training_list, R0, R1, R2, R3, test_list, 9)
